...
cat > welcome.txt << EOF
  ...
  #####################################
  #                                   #
  #  Welcome to the AmayaOS Installer #
  #       Run 'amaya-installer'       #
  #                                   #
  #####################################
...
# Create amayaOS installer
cat > amaya-installer << "EOF"
#!/bin/sh

ls -1 /dev/sd*

echo "Which device would you like to install AmayaOS to?"
read device

echo "Are you sure you want to install AmayaOS to $device ? All the data on this disk will be lost. (y/n)"
read prompt
if [ "$prompt" == "y" ]
then
    dd if=/AmayaOS.iso of="$device"
    echo "Installed AmayaOS to $device"
else
    echo "AmayaOS installation cancelled"
fi

EOF

chmod +x amaya-installer

cd ..

# Copy AmayaOS image
cp ../AmayaOS.iso AmayaOS.iso
